...
from ..crud import decode_token, get_default_column_by_filter, get_default_columns


router = APIRouter(prefix='/default_column')

@router.get("/id/{default_column_id}", response_model=schemas.DefaultColumn)
def get_default_column_by_id(default_column_id: int, db: Session = Depends(get_db), agent_data: schemas.TokenData = Depends(decode_token)):
...
def get_all_default_columns(db: Session = Depends(get_db), agent_data: schemas.TokenData = Depends(decode_token)):
